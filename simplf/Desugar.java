...
public class Desugar implements Expr.Visitor<Expr>, Stmt.Visitor<Stmt> {
    ...
    public Stmt visitForStmt(For stmt) {
        ...
        if (stmt.incr != null) {
            whileBodyStmts.add(new Stmt.Expression(stmt.incr.accept(this)));
        }
        Stmt newBody = new Stmt.Block(whileBodyStmts);

        
        Expr newCond;
        if (stmt.cond == null) {
            newCond = new Expr.Literal(true);
        } else {
            newCond = stmt.cond.accept(this);
        }

        
        Stmt whileLoop = new Stmt.While(newCond, newBody);

        
        if (stmt.init != null) {
            Stmt initStmt = new Stmt.Expression(stmt.init.accept(this));
            return new Stmt.Block(Arrays.asList(initStmt, whileLoop));
        }
